...
bool initializeDatabase()
{
    const QStringList queries = QStringList()
        ...
        << "INSERT INTO views VALUES(5,5,1,'By Publisher',2,'SELECT DISTINCT publisher FROM books ORDER BY upper(publisher), publisher;SELECT DISTINCT file, title, author FROM books WHERE publisher = :publisher ORDER BY upper(title), title','/usr/share/explorer/images/middle/publisher.png')"
        << "INSERT INTO views VALUES(6,6,1,'By Year',2,'SELECT DISTINCT year FROM books ORDER BY year;SELECT DISTINCT file, title, author FROM books WHERE year = :year ORDER BY upper(title), title','/usr/share/explorer/images/middle/calendar.png')"
        << "INSERT INTO views VALUES(7,7,1,'By Series',2,'SELECT DISTINCT series FROM books WHERE series <> '''' ORDER BY upper(series), series;SELECT DISTINCT file, series_index||'' ''||title as title, author FROM books WHERE series = :series ORDER BY series_index','/usr/share/explorer/images/middle/series.png')"
        << "INSERT INTO views VALUES(8,8,1,'Unread',2,'SELECT DISTINCT file, title, author FROM books WHERE read_count = 0 ORDER BY upper(title), title','/usr/share/explorer/images/middle/unread.png')"
